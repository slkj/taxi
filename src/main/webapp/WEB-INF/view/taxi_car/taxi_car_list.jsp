	...
	<script type="text/javascript">
		...
		function editRow(id) {
			if (id) {
				...
				$.ajax({
					type : "post",
					dataType : "json",
					url : '../taxicar/queryOne?id='+id,
					async : true,
					success : function(result) {
						if (result) {
							$("#vui_sample").form("load", result);	
							$("#uploadPreviewOwnerNamePic").attr("src","../taxicar/getOwnernamepic?id=" + id);
							$("#uploadPreviewVehiclePic").attr("src","../taxicar/getVehiclepic?id=" + id);
						} else {
							showError("系统异常");
						}
					}
		...
		function clearForm(){//重置表单
			$('#vui_sample').form('clear');
			 $("#uploadPreviewOwnerNamePic").removeAttr("src");
			 $("#uploadPreviewVehiclePic").removeAttr("src");
		}
		//删除
		function delRow(id) {
			if (confirm("确定要删除吗？")) {
