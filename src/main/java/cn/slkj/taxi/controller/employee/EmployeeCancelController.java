	...
	public ModelAndView examineAdd() {
	 ...
	 @RequestMapping({"/goShow"})
	  public ModelAndView goShow()
	  {
	    ...
	    try
	    {
	    	EmployeeCancel varList = employeeCancelService.selectOne(pd);
	      mv.setViewName("employee_cancel/employee_cancel_show");
	...
	@RequestMapping({"/goExcel"})
	  public ModelAndView goExcel(HttpSession session)
	  {
	    ...
	    try {
	    	...
	    	//注销类型
	    	if ((pd.getString("flag") != null) && (!"".equalsIgnoreCase(pd.getString("flag").trim()))) {
		    	 String flag= URLDecoder.decode(pd.getString("flag"), "utf-8");
		    	 pd.put("flag", flag);
		    	}
	      ...
	      List emList = this.employeeCancelService.excelList(pd);
	      ...
	      for (int i = 0; i < emList.size(); i++) {
	        ...
	        vpd.put("var12", ((PageData)emList.get(i)).getString("COMPANY"));
	        vpd.put("var13", ((PageData)emList.get(i)).getString("PASSTIME"));
	        ...
	        if (((PageData)emList.get(i)).getIntegr("FLAG") != null) {
	        	
		          if (((PageData)emList.get(i)).getIntegr("FLAG")==0)
		            vpd.put("var15", "普通注销");
		          else if (((PageData)emList.get(i)).getIntegr("FLAG")==1)
		            vpd.put("var15", "报废注销");       
		          else
		            vpd.put("var15", "未填写");
		        }
